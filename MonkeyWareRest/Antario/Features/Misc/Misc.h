...
public:
	...
	void doshitnadepredict() {
		...
		if (c_config::get().grenade_prediction)
		{
			...
			*(int*)((DWORD)&sv_cheats->fnChangeCallback + 0xC) = 0; // ew
			...
			g_pEngine->ExecuteClientCmd("cl_grenadepreview 1");
		}
	}
	...
	void FakeDuck() {
		...
		{
				unsigned int fdbind = c_config::get().fakeduck_test;

				if (Globals::LocalPlayer->GetFlags() & FL_ONGROUND) 
				{
					if (fdbind)
						Globals::pCmd->buttons |= IN_DUCK;
					else// hahahha in_duck go brrrrrrrrrr
						Globals::pCmd->buttons &= ~IN_DUCK;
				}
			} 
	}
			...
			if (m_local && m_local->IsAlive()) {
				for (int i = 1; i < g_pGlobalVars->maxClients; i++)
				{
					...
					if (m_entity && m_entity->is_valid(Globals::LocalPlayer)) {
						...
						int choked_ticks = fine(TIME_TO_TICKS(simtime_delta), 1, 16);
						Vector lastOrig;
	...
	void Thirdperson_FSN(ClientFrameStage_t curStage) {
		if (curStage == FRAME_RENDER_START && g_GameInput->m_fCameraInThirdPerson && Globals::LocalPlayer && Globals::LocalPlayer->IsAlive())
		{
			...
			//static int deadflagOffset = g_pNetvars->GetOffset("DT_CSPlayer", "deadflag"); //12756
			//if(deadflagOffset && (Vector*)((DWORD)Globals::LocalPlayer + deadflagOffset + 4))
			//*(Vector*)((DWORD)Globals::LocalPlayer + 12756 + 4) = Vector(Globals::RealAngle.x, Globals::RealAngle.y, 0);
	...
	void Animated_Clantag() {
		...
		static std::string clantag = "Monkeyware               ";
		if (++counter > 25) {
...
private:
	...
	void DoFakeLag()
	{
		...
		if (!Globals::LocalPlayer->IsAlive() || Globals::LocalPlayer->IsNade() || Globals::LocalPlayer->GetActiveWeapon()->GetItemDefinitionIndex() == ItemDefinitionIndex::WEAPON_REVOLVER)
			return;
