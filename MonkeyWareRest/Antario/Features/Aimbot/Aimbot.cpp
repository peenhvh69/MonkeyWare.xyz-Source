...
void Aimbot::OnCreateMove()
{
	...
	if (shot)
	{
		if (c_config::get().on_shot_aa) {
			...
			else if (c_config::get().on_shot_aa_type == 2) {
				Globals::bSendPacket = true;// desyncing so if they aren't resolvering it looks like we're shooting
				Globals::pCmd->viewangles.x = 89;// forcing head down
				Globals::pCmd->viewangles.y = 180;// forcing yaw backwards
			}
