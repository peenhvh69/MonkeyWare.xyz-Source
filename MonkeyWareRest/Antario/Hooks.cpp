...
void Hooks::Init()
{
	...
	g_Hooks.pClientHook		->Hook(vtable_indexes::writeUserCmdToBuf, Hooks::WriteUsercmdDeltaToBufferHook);
...
{
	//Utils::FindPattern("engine.dll", reinterpret_cast<PBYTE>("\xA1\x00\x00\x00\x00\xB9\x00\x00\x00\x00\x56\xFF\x50\x14\x8B\x34\x85"), "x????x????xxxxxxx"));
	static auto WriteUsercmdF = Utils::FindPattern("client.dll", reinterpret_cast<PBYTE>("\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x8B\xD9\x8B\x0D"), "xxxxxxxxxxxxx");
...
bool __fastcall Hooks::WriteUsercmdDeltaToBufferHook(void* ecx, void* edx, int slot, void* buf, int from, int to, bool isnewcommand)
{
	static auto ofunct = g_Hooks.pClientHook->GetOriginal<WriteUsercmdDeltaToBuffer_t>(vtable_indexes::writeUserCmdToBuf);

	if (g_Doubletap.m_tick_to_shift <= 0)
		return ofunct(ecx, slot, buf, from, to, isnewcommand);

	//g_pEngine->ClientCmd_Unrestricted("boom it workie", false);
	...
	auto CL_SendMove = []() {
		...
		//FindPattern("engine.dll", "55 8B EC A1 ? ? ? ? 81 EC ? ? ? ? B9 ? ? ? ? 53 8B 98");
		static CL_SendMove_t CL_SendMoveF = (CL_SendMove_t)Utils::FindPattern("engine.dll", reinterpret_cast<PBYTE>("\x55\x8B\xEC\xA1\x00\x00\x00\x00\x81\xEC\x00\x00\x00\x00\xB9\x00\x00\x00\x00\x53\x8B\x98"), "xxxx????xx????x????xxx");
	...
	int32_t total_new_commands = min(g_Doubletap.m_tick_to_shift, 16); // assuming 8 shift ticks
	g_Doubletap.m_tick_to_shift -= total_new_commands;
	g_Doubletap.m_tick_to_recharge += total_new_commands;
	...
	*pNumBackupCommands = 0;
	...
	for (to = next_cmdnr - new_commands + 1; to <= next_cmdnr; to++) {
		//(IClientMode*, void*, float, CUserCmd*);
...
void __fastcall Hooks::PaintTraverse(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	if (c_config::get().removers[0] && c_config::get().visuals_enabled && !strcmp("HudZoom", g_pPanel->GetName(vguiPanel)))
		return;
	...
	if (panelID == vguiPanel) 
	{
		...
		Color semen(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, 255);// this is like a watermark but not really at the same time it just renders text in the top left

		g_pSurface->DrawT(c_config::get().watermark_x_value, c_config::get().watermark_y_value, semen, Globals::CourierNew, 0, "MonkeyWare.xyz");// we put it there to get it out of the radar's way
...
void __fastcall Hooks::OverrideView(void* ecx, void* edx, CViewSetup* pSetup)
{
	...
	if (g_pEngine->IsConnected() && g_pEngine->IsInGame())
	{
		...
		if (c_config::get().c_config::get().removers[1])
		{
...
void __fastcall Hooks::SceneEnd(void *ecx, void *edx)
{
	...
	for (int i = 1; i < g_pEngine->GetMaxClients(); ++i)
	{
		...
		*/
		...
		float local_fake_color[3] = { c_config::get().local_fake_chams_r / 255.f, c_config::get().local_fake_chams_g / 255.f, c_config::get().local_fake_chams_b / 255.f };
		...
		else if (IsLocal && !Globals::LocalPlayer->IsScoped() && c_config::get().local_fake_chams) {
			g_pRenderView->SetBlend(c_config::get().local_fake_chams_a / 100.f);
			g_pModelRender->ForcedMaterialOverride(materials.at(0));
