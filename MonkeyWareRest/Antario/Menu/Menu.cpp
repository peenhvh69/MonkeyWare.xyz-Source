...
void DrawESPPlayersSubTab(int& current_players_esp_subtab, int tab_amount, Vector _pos, int MenuAlpha_Main)
{
	...
	static std::string ESP_Player_SubTabs_Names[3] = {"Main", "Chams", "Indecators"};
	...
	for (int i = 0; i < tab_amount; i++)
	{
		...
		if (current_players_esp_subtab == i)
		{
			...
			g_pSurface->FilledRect(tab_area[0], tab_area[1] + tab_area[3], tab_area[2], 2,  Color(c_config::get().menu_color_r, c_config::get().menu_color_g, c_config::get().menu_color_b, MenuAlpha_Main));
...
{
	int in_sizew_esp_player_subtabs = GroupBoxSize_Width - 8;
	static std::string ESP_Player_SubTabs_Names[2] = {"Main", "Hahahah ticks go brrrrr"};
...
std::string config_slots[7] = {
"mm", "wm", "hvh", "nospread", "custom", "custom2", "custom3"
};
...
void Menu::Render()
{
	...
	RECT TitleText_Size = g_pSurface->GetTextSizeRect(Globals::CourierNew, "Monkeyware.xyz");

	g_pSurface->DrawT(_pos.x + 3, _pos.y + (20 / 2) - (TitleText_Size.bottom / 2), Color(143, 143, 143, MenuAlpha_Text), Globals::CourierNew, false, "Monkeyware.xyz");
	...
	static std::string tab_names[tab_amount] = { "Rage", "AA", "Visuals", "Misc", "PlayerList" };
	...
	std::string ResolverOptions[2] = { "Logic", "Brute" };
	std::string BodyAimModeOptions[2] = { "Prefer", "Force" };
	std::string HitboxOptions[7] = { "Head", "Upper Chest", "Stomach", "Pelvis", "Arms", "Legs", "Toes" };
	 ...
	 if (tab_selected == 0)
	{
		...
		if (aimbot_main_subtab == 0)
		{
			...
			if (c_config::get().aimbot_resolver) combobox(2, "Resolver Mode", ResolverOptions, &c_config::get().resolver_mode);
		...
		else if (aimbot_main_subtab == 1)
		{
			...
			if (c_config::get().doubletap) slider(16, "DT", &c_config::get().ticks_to_shift, "Ticks ", 1);
			checkbox("HideShots", &c_config::get().hideshots);
		}
		...
		case 0:
		{
			...
			MultiComboBox(7, "Hitboxes", HitboxOptions, &c_config::get().auto_hitbox);
		}
		break;
		case 1:
		{
			slider(100, "Hitchance", &c_config::get().scout_hitchance, "%", 1);
			slider(120, "Minimum Damage", &c_config::get().scout_mindamage, "HP", 1);
			MultiComboBox(7, "Hitboxes", HitboxOptions, &c_config::get().scout_hitbox);
		}
		break;
		case 2:
		{
			slider(100, "Hitchance", &c_config::get().awp_hitchance, "%", 1);
			slider(120, "Minimum Damage", &c_config::get().awp_mindamage, "HP", 1);
			MultiComboBox(7, "Hitboxes", HitboxOptions, &c_config::get().awp_hitbox);
		}
		break;
		case 3:
		{
			...
			MultiComboBox(7, "Hitboxes", HitboxOptions, &c_config::get().heavy_pistol_hitbox);
		}
		break;
		case 4:
		{
			...
			MultiComboBox(7, "Hitboxes", HitboxOptions, &c_config::get().other_hitbox);
		}
		break;
		}
		...
		std::string pitch[4] = { "Off", "Down", "Up", "Random" };
		std::string yaw[4] = { "Off", "Backwards", "Slightly down", "Spin" };
		std::string on_shot[3] = { "Stable", "Experimental", "Hide Shots" };
		std::string desync[5] = { "Off", "Left/Right switching", "Middle/right", "Middle/left", "Fucking 180 jitter" };
		...
		case 0:
		{
			combobox(4, "Pitch", pitch, &c_config::get().pitch_standing);
			combobox(4, "Yaw", yaw, &c_config::get().yaw_standing);
		}
		break;
		case 1:
		{
			combobox(4, "Pitch", pitch, &c_config::get().pitch_moving);
			combobox(4, "Yaw", yaw, &c_config::get().yaw_moving);
		}
		break;
		case 2:
		{
			combobox(4, "Pitch", pitch, &c_config::get().pitch_air);
			combobox(4, "Yaw", yaw, &c_config::get().yaw_air);
		}
		break;
		case 3:
		{
			...
			{
				combobox(3, "On Shot AA Type", on_shot, &c_config::get().on_shot_aa_type);
			}
	...
	else if (tab_selected == 2)
	{
		...
		std::string RemoverOptions[4] = { "No Scope", "No Visual Recoil", "No Smoke", "No Flash" };
		...
		MultiComboBox(4, "Removers", RemoverOptions, c_config::get().removers);
		checkbox("Hit Marker", &c_config::get().hitmarker);
		if (c_config::get().hitmarker) combobox(3, "Hit Marker Sound", Hitmarker_Type, &c_config::get().hitmarker_sound);
		{
		...
		{
			checkbox("!!! THIS TURNS ESP ON !!!", &c_config::get().visuals_enabled);
		...
		else if (current_players_esp_subtab == 1)
		{
			...
			//local_fake_chams_r
			color_selector("local_fake_col", &c_config::get().local_fake_chams_r, &c_config::get().local_fake_chams_g,
				&c_config::get().local_chams_color_b, &c_config::get().local_chams_color_a);
			checkbox("Fake Chams", &c_config::get().local_fake_chams);
	...
	else if (tab_selected == 3)
	{
		...
		groupbox(TopLeftGroupBox_X, TopLeftGroupBox_Y, GroupBoxSize_Width, GroupBoxSize_Height, "test");
		std::string KeyBindList[4] = { "Thirdperson", "Slow Walk", "Swap Manual", "Fakeduck" };
		...
		//slider(100, "Slow Walk", &c_config::get().slowwalk_speed, "%", 1);
		slider(1920, "watermark y", &c_config::get().watermark_y_value, "", 1);
		slider(2160, "watermark x", &c_config::get().watermark_x_value, "", 1);
...
void Menu::slider(int max, std::string name, int* item, std::string suffix, int step)
{
	...
	// g_pSurface->DrawT(x_offset + 18, y_offset + 11, Color(100, 100, 100, MenuAlpha_Main), Globals::SmallText, false, "-");
		// these are cool, adds a - at the start and a + at the end to sliders. useless but cool!
	// g_pSurface->DrawT(x_offset + 21 + SliderSize+ 20, y_offset + 11, Color(100, 100, 100, MenuAlpha_Main), Globals::SmallText, false, "+");
...
void Menu::color_selector(std::string name, int* red, int* green, int* blue, int* alpha)
{
	...
	/*
	Color rainbow; //total didint just un // this area i sware!!!!11!1!1
